...
class Konsultasiuseetv extends CI_Controller
{
    ...
    */
        ...
        $symptom = $this->Useetv_model->getGejalaByKode($this->open[0]);
    ...
    public function transition()
    {
        ...
        if ($isYes == '1') {
            ...
            $this->session->unset_userdata('open_');
            $this->session->unset_userdata('root_');
            redirect('konsultasiuseetv/percentage');
        } else {
            ...
            if ($this->session->userdata('temporary_roots_') === $this->emptyArray) {
                ...
                if (count($open) == 0) {
                    ...
                    $this->session->unset_userdata('open_');
                    $this->session->unset_userdata('root_');
                    redirect('konsultasiuseetv/unknownresult');
                }
                ...
                redirect('konsultasiuseetv/questions/' . $number);
            }
    ...
    public function percentage()
    {
        if (!$this->session->userdata('percentage_') || !$this->session->userdata('symptomCode_')) {
            redirect('konsultasiuseetv/questions/1');
        }
        $data['title'] = "Gangguan UseeTV";
        $data['user'] = $this->db->get_where('users', ['email' => $this->session->userdata('email')])->row_array();
        $data['gejalaByGangguan'] = $this->Useetv_model->gejalaByGangguan2();
    ...
    public function result()
    {
        ...
        for ($i = 0; $i < count($allGejala); $i++) {
            for ($j = 0; $j < count($allGejala[$i]); $j++) {
                $cf = $this->Useetv_model->getGejalaByKode($allGejala[$i][$j][1]);
