...
class Konsultasiuseetv extends CI_Controller
{
    ...
    public function percentage()
    {
        ...
        $data['user'] = $this->db->get_where('users', ['email' => $this->session->userdata('email')])->row_array();
        $data['requests'] = $this->Request_model->getAllData();
        $allGejalaWithGangguan = $this->Useetv_model->gejalaByGangguan2();
        $allGejala = $this->Useetv_model->getAllGejala();
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('konsultasiuseetv/percentage', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function request()
    {
        ...
        $this->Useetv_model->requestGejala($id_user);
        if ($data['user']['role_id'] != 1) {
            $this->session->set_flashdata('flash', 'Diajukan ke Pakar');
        } else {
            $this->session->set_flashdata('flash', 'Ditambahkan');
        }
        redirect('konsultasiuseetv/diagnosa');
    }
}
